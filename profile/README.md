...
- 레디스 세션 클러스터링을 활용하여 장바구니 항목 개수 유지 및 뷰 헤더의 장바구니 항목 개수를 동적으로 최신화하여 표현
...
- 리펙토링
    - 게스트 장바구니=Redis, 회원 장바구니=DB -> 게스트/회원=Redis로 통합하여 성능 개선
    - 일정 주기 마다 스케줄러로 회원 장바구니 DB에 적용하여 Redis의 영구성 문제 해결
...
- 비회원/회원 및 관리자의 주문 내역에 대한 조회
- 관리자의 주문 배송 시작
- 배송 시작 전 상품에 대한 회원의 주문 취소 처리(외부 결제 API의 결제 취소)
- 배송 완료 상품에 대해 일정 기간내에서 회원의 반품 요청 처리
